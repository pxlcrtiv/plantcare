...
- [UI redesign to reference screenshot](https://github.com/pxlcrtiv/plantcare/issues/12) — **grilled 2026-08-16**: restyle *everything* (onboarding/login/add-plant/profile too), replace the 4-tab bar with the new 5-tab dock (Home / Plant / Search / Flask / Target; Calendar + Profile move to the dashboard header menu), wire metrics + greeting from the plant model and auth user (no mocks), keep Inter + Unsplash images, keep light+dark (dark derived from new palette), actions wired where cheap, base `run/first-boot`. New palette: bg `#F5F5F0`, accent olive `#8CB23E`, white cards, large radii + pills. Ticket has the full screenshot spec.
